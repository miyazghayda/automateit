...
class FollowinghashtagCommand extends Command {
    public $Accounts;
    public $Hashtaglists;
    ...
    public function initialize() {
        ...
        $this->Preferences = $this->loadModel('Preferences');
    ...
    public function execute(Arguments $args, ConsoleIo $io) {
        while (true) {
            ...
            $query->matching('Preferences', function($q) {
                return $q->where(['Preferences.followbyhashtag' => true, 'Preferences.gethashtagtofollowtoday' => true]);
            });

            if ($query->count() > 0) {
                $accounts = $query->all();
                foreach ($accounts as $account) {
                    // Count how much maximum follow per day by hashtags
                    $divider = 1;
                    if ($account['preferences'][0]['followidolfollower']) $divider++;
                    if ($account['preferences'][0]['followbylocation']) $divider++;
                    $maxfollowbyhashtagperday = floor($account['preferences'][0]['maxfollowperday'] / $divider);

                    $totalFollowPerHashtag = floor($maxfollowbyhashtagperday / count($account['hashtaglists']));
                    ...
                    try {
                        ...
                        foreach ($account['hashtaglists'] as $hashtag) {
                            ...
                            //                      ig object,  hashtag to search,      how much posts, account id, preference id
                            $this->getHashtagFeed($ig,        $hashtag['caption'],    $totalFollowPerHashtag,             $account['id'], $account['preferences'][0]['id']);
                        }// .foreach hashtag
                    } catch(Exception $e) {
                        $io->out($e);
                    }// .try login ig
            ...
            // Delay per script call, do not remove
            sleep($this->delayed);
    ...
    private function getHashtagFeed($ig = null, $tag = 'exploreindonesia', $maxPostToGet = 20, $account_id = 1, $preference_id = 1) {
        $io = new ConsoleIo();
        ...
        do {
            ...
            foreach ($response->getItems() as $item) {
                ...
                // Insert to followinglists table to queue to follow
                $followinglist_id = $this->insertFollowinglist($ig, $account_id, $member_id, $item->getUser());
            }
    ...
    private function insertFollowinglist($ig = null, $account_id = 1, $member_id = 1, $userIg = null) {
        $io = new ConsoleIo;
        $check = $this->Followinglists->find()
                      ->where(['vassal_id' => $member_id, 'account_id' => $account_id, 'active' => true]);

        if ($check->count() == 1) {
            $ret = $check->first();
            return $ret->id;
        } else {// follow and save post to db
                try {
                    //$io->out("Memfollow {$userIg->getUsername()}");
                    $following = $ig->people->follow($userIg->getPk());
                    if ($following->getStatus() == 'ok') {
                        $io->out("Berhasil memfollow {$userIg->getUsername()}");
                        $data = [
                            'vassal_id' => 1,
                            'member_id' => $member_id,
                            'fellow_id' => $member_id,
                            ...
                            'followed' => true,
                            'unfollowed' => false,
                            'followedat' => Time::now()->i18nFormat('yyyy-MM-dd HH:mm:ss'),
                            'who' => false,
                        ...
                        $followinglist = $this->Followinglists->patchEntity($followinglist, $data);
                        $this->Followinglists->save($followinglist);

                        // todo create mysql stored procedure to update preferences table
                        // Update preferences table
                        $additionalExpression = $this->Preferences->query()->newExpr('hashtagtofollowtoday + 1');
                        $affectedRow = $this->Preferences->updateAll(
                            ['hashtagtofollowtoday' => $additionalExpression],
                            ['account_id' => $account_id]
                        );
                        // Count how much maximum follow per day by hashtags
                        ...
                        if ($account['preferences'][0]['followbylocation']) $divider++;
                        $maxfollowbyhashtagperday = floor($account['preferences'][0]['maxfollowperday'] / $divider);
                        $maxfollowperday = $account['preferences'][0]['maxfollowperday'];
                        $hashtagtofollowtoday = $account['preferences'][0]['hashtagtofollowtoday'];
                        if ($hashtagtofollowtoday > $maxfollowperday || $hashtagtofollowtoday > $maxfollowbyhashtagperday) {
                            $query = $this->Preferences->query();
                            $query->update()
                                  ->set(['gethashtagtofollowtoday' => false])
                                  ->where(['account_id' => $account_id])
                                  ->execute();
                        }
                        sleep($this->followDelay);
                    }
                } catch(Exception $e) {
                    $io->out($e);
                }
        }
    }// .private function insertFollowinglist
